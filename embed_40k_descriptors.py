...
output_file = "data/processed/descriptors_with_explainers_embeddings_40k.jsonl"
...
# First, collect all unique descriptors from first 40k lines
print(f"Collecting unique descriptors from first {MAX_LINES} lines of input file...")
unique_descriptors = set()
total_docs = 0

with open(input_file, "r") as in_f:
    for line_idx, line in enumerate(tqdm.tqdm(in_f)):
        # Stop after processing MAX_LINES
...
print(
    f"Found {len(unique_descriptors)} unique descriptors from {total_docs} documents (first {MAX_LINES} lines)"
)
...
print(
    f"Done! Embedded {len(unique_descriptors)} unique descriptors from first {MAX_LINES} lines."
)
